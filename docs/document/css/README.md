...
* 阻止块级元素与浮动元素的覆盖(创建BFC后，p不与浮动元素重叠，文字也不会围绕浮动元素)[隔离](http://js.jirengu.com/cihiw/1/edit)
...
reset是暴力清除标签默认样式，比如 * {margin:0; padding: 0}，normalize保留默认样式，仅修复样式在不同浏览器上的差异。个人见解：理论上normalize更好，实际选择中其实差别不大。
